skip_to([], _, []) :- !.
skip_to([X|Xs], X, Xs) :- !.
skip_to([_|Xs], X, Ys) :- skip_to(Xs, X, Ys).

check_abba(['['|Xs]) :- !, skip_to(Xs, ']', Ys), check_abba(Ys).
check_abba([A,B,B,A|_]) :- A \= B, !.
check_abba([_|Xs]) :- check_abba(Xs).

check_no_abba([]) :- !.
check_no_abba([']'|Xs]) :- !, skip_to(Xs, '[', Ys), check_no_abba(Ys).
check_no_abba([A,B,B,A|_]) :- A \= B, !, fail.
check_no_abba([_|Xs]) :- check_no_abba(Xs).

has_tls(S) :-
    atom_chars(S, Xs),
    check_abba(Xs),
    skip_to(Xs, '[', Ys), check_no_abba(Ys).

adv07(X) :- data(D), include(has_tls, D, D1), length(D1, X).

check_aba(['['|Xs], A, B) :- !, skip_to(Xs, ']', Ys), check_aba(Ys, A, B).
check_aba([A,B,A|_], A, B) :- A \= B.
check_aba([_|Xs], A, B) :- check_aba(Xs, A, B).

check_bab([']'|Xs], A, B) :- !, skip_to(Xs, '[', Ys), check_bab(Ys, A, B).
check_bab([B,A,B|_], A, B) :- A \= B.
check_bab([_|Xs], A, B) :- check_bab(Xs, A, B).

has_ssl(S) :-
    atom_chars(S, Xs),
    check_aba(Xs, A, B),
    skip_to(Xs, '[', Ys), check_bab(Ys, A, B).

adv07b(X) :- data(D), include(has_ssl, D, D1), length(D1, X).
